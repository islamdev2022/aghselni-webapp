...
export default function AppointmentTabs() {
  ...
  const { data: allAppointments, isLoading: isAllLoading } =
    useGetInternAppointmentsClient();

  // Update appointment status mutation
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: number; status: string }) => {
      const response = await api.put(`/api/appointments_location/${id}/`, {
        status,
      });
  ...
  const displayAppointments =
    activeTab === "pending" ? pendingAppointments : completedAppointments;
                ...
                <div className="rounded-lg bg-gray-50 p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium text-gray-700">
                        {selectedAppointment.car_name} (
                        {selectedAppointment.car_type})
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-cyan-100 text-cyan-700 overflow-hidden">
                      {selectedAppointment.client_info.photo ? (
                        <img
                          src={
                            `http://127.0.0.1:8000${selectedAppointment.client_info.photo}` ||
                            "/placeholder.svg"
                          }
                    ...
                    <div className="ml-4">
                      <p className="text-sm font-medium text-gray-900">
                        {selectedAppointment.client_info.full_name}
                      </p>
          ...
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
            ...
            <div className="mb-6">
              <p className="mb-4 text-sm text-gray-600">
                Update the status for appointment #{selectedAppointment.id} -{" "}
                {selectedAppointment.car_name} ({selectedAppointment.wash_type})
              </p>
